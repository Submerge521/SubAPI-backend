-- 创建库
create database if not exists subapi;

-- 切换库
use subapi;

create table if not exists subapi.daily_check_in
(
...
create table if not exists subapi.interface_info
(
    ...
    returnFormat   varchar(512) default 'JSON'            not null comment '返回格式（如JSON）',
    description    varchar(256)                           null comment '描述',
    status         int          default 0                 not null comment '接口状态（0-关闭，1-开启）',
    totalNum       bigint       default 0                 null comment '接口总调用次数',
...
create table if not exists subapi.post
(
    id            bigint auto_increment comment 'id'
        primary key,
...
create table if not exists subapi.user
(
    id             bigint auto_increment comment 'id'
        primary key,
...
create table if not exists subapi.user_interface_info
(
    id              bigint auto_increment comment '主键'
        primary key,
